...
### Changed
-   **Performance**: Optimized data fetching on the vessel profile page by using `Promise.all` to fetch vessel, crew, and inventory data concurrently, resulting in a faster load time.
...
### Changed
-   **AI Enhancement**: Upgraded the AI Video Generation flow to include ambient sound by adding descriptive text to the prompts, creating a more immersive experience.
...
### Changed
-   **Reporting**: Added an "Export to CSV" feature to the Crew Manifest, Vessel Status, and Certificate Status reports to allow users to easily work with data in external tools.
